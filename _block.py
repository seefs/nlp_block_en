...
def _m_to_mts(inputs):
    m1, m2, l1, l2 = inputs
    # 4x9 mode; add m = 2/4/6/8 compatible; remove m = 5 in the last group (at the same time repeat 8 for alignment)
...
#############
    ...
    def check_in_block(_info, _m, _i, _len):
        ...
        ret = tf.cond(tf.logical_and(_i<_len, tf.logical_or(tf.equal(_item,0), tf.equal(_item,26))), lambda:0, lambda:ret)         #Mode 0/26 is not divided into blocks
...
#############
    ...
    while step<500: 
        step += 1
        # ret=0,1,2,3;4,5: Left, right, single left fetch block, single right fetch block; there are remaining blocks, out of range
        ret = tf.cond(tf.logical_and(tf.greater_equal(l,l1), tf.greater_equal(r,l2)), lambda:update_state(5, l, r), lambda:ret)
        # ret=4,5: There are remaining blocks, out of range
        ...
        # s_left=0,1,2:  Left block is in block, not in block, out of range
        # s_right=0,1,2: Right block is in block, not in block, out of range
        ...
        # ret=0,1,2,3;4,5,6: Left, right, single left block, single right block; no remaining blocks, there are remaining blocks, out of range
...
def get_m2n_from_seq(k1, k2, max_space=4, debug=False):
    s_list = [ [15, 16,  7,  9,  3, 10, 11],
               [15, 14,  7, 13,  3, 12, 11], 
               [15, 16,  7,  9,  8,  1,  2,  9,  3, 10],  # Don't last 11, it's too far
               [15, 14,  7, 13,  6,  5,  4, 13,  3, 12],  # Don't last 11, it's too far
